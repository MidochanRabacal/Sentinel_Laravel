...
</nav><!-- added glyphicons April 04 '17 -->

<table class = "table">
    <tr>
      <td>Date</td><!-- change Available Menu into Date -->
      @if($week < 10)
        <?php
          $week = '0'. $week;
        ?>
    ...
    <tr>
      ...
      @foreach($breakfasts as $breakfast)
        ...
        <?php
                  $d = strtotime($year ."W". $week . $day);
                  echo "<td id='".date('Y-m-d', $d)."'> {$breakfast->menuName}<br>₱ {$breakfast->menuPrice}<br>{$breakfast->menuCatName} </td>";
      ...
      @endforeach
        ...
        <tr>
          ...
          @for($day= 1; $day <= 7; $day++)
              <?php
                $d = strtotime($year ."W". $week . $day);
                echo "<td id='".date('y-m-d', $d)."'> {$dinner->menuName}<br>₱ {$dinner->menuPrice}<br>{$dinner->menuCatName}</td>";
